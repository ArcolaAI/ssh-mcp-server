...
export function registerExecuteCommandTool(server) {
    ...
    server.registerTool("execute-command", {
        ...
        inputSchema: {
            ...
            timeout: z
                .number()
                .optional()
                .describe("Command execution timeout in milliseconds (optional; overrides the connection's commandTimeoutMs or shellCommandTimeoutMs, which defaults to 30000ms)"),
        },
    }, async ({ cmdString, directory, connectionName, timeout }) => {
        try {
            const result = await sshManager.executeCommand(cmdString, directory, connectionName, {
                timeout,
            });
            return {
