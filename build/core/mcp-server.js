...
export class SshMcpServer {
        ...
        if (!this.shutdownPromise) {
            this.shutdownPromise = (async () => {
                ...
                try {
                    ...
                    Logger.log(`Failed to close MCP server cleanly: ${error.message}`, "error");
                }
            })();
        }
    ...
    async run() {
        ...
        if (allConfigs.some((c) => (c.transportMode || "exec") === "exec" &&
            (!c.allowedRemotePaths || c.allowedRemotePaths.length === 0))) {
            Logger.log("WARNING: Running without allowedRemotePaths is strongly discouraged. SFTP upload/download can read or write any path on the remote server. Configure allowedRemotePaths to restrict the SFTP surface.", "info");
        }
