 ...
 */
export function lookupSshConfig(hostAlias, configFilePath) {
 ...
 * 解析 SSH 配置文件
 */
function parseConfigFile(filePath, visited) {
    // 防止循环引用
 ...
 */
function expandIncludePath(pattern, baseDir) {
    // 展开 ~
 ...
 * 匹配主机别名
 */
function matchHost(hostAlias, blocks) {
    const result = {};
...
function hostBlockMatches(hostAlias, patterns) {
...
function hostPatternMatches(hostAlias, pattern) {
    if (pattern === '*') {
        return true;
    }
