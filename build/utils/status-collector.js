 ...
 */
function buildStatusScript(commands, marker) {
    const probes = Object.entries(commands).map(([field, command]) => `printf '\\n${marker}${field}\\n'; { ${command}; } 2>/dev/null`);
    return `${probes.join("; ")}; true`;
}
function parseStatusScriptOutput(output, marker) {
 ...
 */
export async function collectSystemStatus(runCommand, connectionName, isCommandAllowed = () => true) {
    const status = {
        reachable: true,
    ...
    try {
        ...
        const allowedCommands = Object.fromEntries(Object.entries(commands).filter(([, command]) => isCommandAllowed(command, connectionName)));
        ...
        try {
            ...
            values = parseStatusScriptOutput(await runCommand(buildStatusScript(allowedCommands, marker), connectionName), marker);
        }
        catch {
        ...
        // Parse results
        const readField = (field) => values.get(field) ?? "";
