...
import { findHostEntries, loadKnownHosts, serverHostKeyAlgorithmsFor, verifyOfferedKey, } from "../utils/known-hosts.js";
import fs from "fs";
...
export class SSHConnectionManager {
        ...
        const connectionPromise = new Promise(async (resolve, reject) => {
            ...
            client.on("error", (err) => {
                ...
                if (hostKeyRejection) {
                    ...
                    rejectOnce(new ToolError("HOST_KEY_REJECTED", `SSH connection [${key}] refused: ${hostKeyRejection}`, false));
                    return;
                }
     ...
     */
    applyHostKeyVerification(key, config, sshConfig) {
        if (config.hostKeyVerification === "off") {
            Logger.log(`[${key}] Host-key verification is OFF by configuration - the host key of ${config.host}:${config.port} will not be checked`, "error");
            return;
        }
        let loaded;
        try {
            ...
            throw new ToolError("HOST_KEY_UNVERIFIABLE", `SSH connection [${key}] refused: cannot read known_hosts (${error.message}). Host-key verification is fail-closed; connect once with the OpenSSH client (ssh ${config.username}@${config.host}) to record the host key, or set hostKeyVerification to "off" to accept any host key.`, false);
        }
        ...
        if (hostEntries.filter((e) => e.marker !== "cert-authority").length === 0) {
            throw new ToolError("HOST_KEY_UNVERIFIABLE", `SSH connection [${key}] refused: ${config.host}:${config.port} has no entry in ${loaded.path}. Host-key verification is fail-closed; connect once with the OpenSSH client (ssh -p ${config.port} ${config.username}@${config.host}), check the fingerprint it shows, and accept it there - this tool never adds host keys itself.`, false);
        }
        const existingAlgorithms = sshConfig.algorithms ?? {};
        ...
        sshConfig.hostVerifier = (offeredKey) => {
            const verdict = verifyOfferedKey(hostEntries, offeredKey);
            if (verdict.ok) {
                Logger.log(`[${key}] Host key verified: ${verdict.keyType} matches ${loaded.path} line ${verdict.lineNumber}`, "info");
                return true;
            }
    ...
    validateCommand(command, name) {
        ...
        if (whitelistRegexes.length > 0) {
            ...
            if (/[;&|`<>\r\n]|\$\(/.test(command)) {
                return {
                    isAllowed: false,
                    reason: "Command contains shell control syntax forbidden by the whitelist",
                };
            }
