...
export type HostKeyVerificationMode = "known_hosts" | "off";
...
export type HostKeyVerdict =
  | { ok: true; keyType: string; lineNumber: number }
  | {
      ok: false;
      reason: "unknown-host" | "revoked" | "mismatch";
      detail: string;
    };
...
function isWireBlobOfType(blob: Buffer, keyType: string): boolean {
  if (blob.length < 4) {
    return false;
  }
...
export function knownHostsLookupName(host: string, port: number): string {
 ...
 */
export function entryMatchesHost(
...
export function findHostEntries(
 ...
 */
export function verifyOfferedKey(
  hostEntries: KnownHostEntry[],
  offeredKey: Buffer,
): HostKeyVerdict {
  ...
  const knownTypes = Array.from(
    new Set(usable.filter((e) => e.marker === undefined).map((e) => e.keyType)),
  );
...
function wireBlobType(blob: Buffer): string | undefined {
  if (blob.length < 4) {
    return undefined;
  }
  const len = blob.readUInt32BE(0);
 ...
 */
export function serverHostKeyAlgorithmsFor(hostEntries: KnownHostEntry[]): string[] {
  const types = new Set(
    hostEntries.filter((e) => e.marker === undefined).map((e) => e.keyType),
  );
...
export interface LoadedKnownHosts {
  path: string;
  entries: KnownHostEntry[];
}

/** Read and parse a known_hosts file. Throws (fail-closed) if it cannot be read. */
export function loadKnownHosts(filePath?: string): LoadedKnownHosts {
